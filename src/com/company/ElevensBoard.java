...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		return findPairSum11(cIndexes) || findsJQK(cIndexes);
	}
	 ...
	 */
	private boolean findsJQK(List<Integer> selectedCards) {
	...
	private boolean playPairSum11IfPossible() {
		...
		for (int i = 0; i < 9; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				if ((cardAt(i).pointValue() + cardAt(j + 1).pointValue())== 11)
				{
					 tf = true;
				}
			}
	...
	private boolean playJQKIfPossible() {
		...
		for (int i = 0; i < 9; i ++)
		{
			al.add(cardAt(i).rank());
		}
		if (al.contains("jack") && al.contains("queen") && al.contains("king"))
		{
			tf = true;
		}
		return true;
